				...
				<div class="row">
					<div class="col-3 mb-3">
    					...
    					<select class="form-select" name="rating">
							<option value="">--</option>
							<option value="10" @selected(($populateData['rating']??'')==10)>10</option>
							@for ($i = 9; $i >= 1; $i--)
								<option value="{{ $i }}" @selected(($populateData['rating']??'')==$i)>{{ $i }}+</option>
							@endfor						
						</select>
					</div>
					<div class="col-3 mb-3">
						...
						<div class="input-group">
							<input type="text" class="form-control" name="year_from" value="{{ $populateData['year_from']??'' }}">
							<input type="text" class="form-control" name="year_to" value="{{ $populateData['year_to']??'' }}">
						</div>
					</div>
					<div class="col-3 mb-3">
						<label for="time_from" class="form-label">{{__('Running time')}}</label>
						<div class="input-group">
							<input type="text" class="form-control" name="time_from" value="{{ $populateData['time_from']??'' }}">
							<input type="text" class="form-control" name="time_to" value="{{ $populateData['time_to']??'' }}">
						</div>
					</div>
					<div class="col-3 mb-3">
